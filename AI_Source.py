...
USE_MOCK_AI = False #flip to False when ready to use gemini or True when wanting to use MOCK mode

client = None
if not USE_MOCK_AI:
    from google import genai
    API_KEY = "AIzaSyCCvV9OVQNQJHowIl10VPTGlNXcY8Dm9sU"#set you API key here!!!!
    if not API_KEY:
        raise RuntimeError("GOOGLE_API_KEY not set! Use: setx GOOGLE_API_KEY 'your_key'")
    client = genai.Client(api_key=API_KEY)
    print("[AI] Running in GEMINI mode.")
else:
    print("[AI] Running in MOCK mode.")
